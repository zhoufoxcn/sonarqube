...
public class DBSessionsImplTest {
  ...
  public void openSession_with_caching_always_returns_the_same_regular_session_when_parameter_is_false() {
    ...
    Set<DbSession> dbSessions = IntStream.range(0, 1 + Math.abs(random.nextInt(10))).mapToObj(ignored -> underTest.openSession(false)).collect(MoreCollectors.toSet());
    assertThat(dbSessions).hasSize(1);
  }
  ...
  public void openSession_with_caching_always_returns_the_same_batch_session_when_parameter_is_true() {
    ...
    Set<DbSession> dbSessions = IntStream.range(0, 1 + Math.abs(random.nextInt(10))).mapToObj(ignored -> underTest.openSession(true)).collect(MoreCollectors.toSet());
    assertThat(dbSessions).hasSize(1);
  }
